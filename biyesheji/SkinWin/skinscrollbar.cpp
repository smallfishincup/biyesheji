...
RECT CSkinScrollBar::GetImageRect(UINT uSBCode,int nState)
{
	...
	if(nIndex == 7)
	{
	RECT rcRet1={m_nWid*nIndex,m_nWid*nState,m_nWid*(nIndex+1)-2,m_nWid*(nState+1)};
	return rcRet1;
	}
	else
	{
		RECT rcRet2={m_nWid*nIndex,m_nWid*nState,m_nWid*(nIndex+1),m_nWid*(nState+1)};
	return rcRet2;
	}
}
...
void CSkinScrollBar::DrawThumb(CDC *pDestDC, RECT *pDestRect, CDC *pSourDC, RECT *pSourRect)
{
	...
	if(IsVertical())
	{
		...
		rcSour.top+=10,rcSour.bottom-=4;
		TileBlt(pDestDC,&rcDest,pSourDC,&rcSour);

		if(pDestRect->bottom-pDestRect->top >= 20)
		{
			rcSour.bottom = rcSour.top;
			rcSour.top-=5;
			rcDest.top += ((rcDest.bottom - rcDest.top) - (rcSour.bottom - rcSour.top))/2;
			rcDest.bottom = rcDest.top + (rcSour.bottom - rcSour.top);
			TileBlt(pDestDC,&rcDest,pSourDC,&rcSour);
		}
	}else
	{
		...
		rcSour.left+=12,rcSour.right-=4;
		TileBlt(pDestDC,&rcDest,pSourDC,&rcSour);

		if(pDestRect->right-pDestRect->left >= 20)
		{
			rcSour.right = rcSour.left;
			rcSour.left-=7;
			rcDest.left += ((rcDest.right - rcDest.left) - (rcSour.right - rcSour.left))/2;
			rcDest.right = rcDest.left + (rcSour.right - rcSour.left);
			TileBlt(pDestDC,&rcDest,pSourDC,&rcSour);
		}
	}
}
