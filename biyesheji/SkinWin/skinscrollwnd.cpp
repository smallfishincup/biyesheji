	...
	if(msg==WM_ERASEBKGND)
	{
			...
			if(dwStyle&WS_VSCROLL)
			{
					...
					||si.nPage!=pSkin->m_sbVert.m_si.nPage)
				{
					...
					pSkin->m_sbVert.EnableWindow(si.nMax>=(int)si.nPage);
				}
			}
			if(dwStyle&WS_HSCROLL)
			{
					...
					||si.nPage!=pSkin->m_sbVert.m_si.nPage)
				{
					...
					pSkin->m_sbHorz.EnableWindow(si.nMax>=(int)si.nPage);
				}
			}
...
BOOL CSkinScrollWnd::SkinWindow(CWnd *pWnd,HBITMAP hBmpScroll)
{
	...
	CRect wndRct = rcFrm;
	m_wndLimit.Create(NULL,"LIMIT",WS_CHILD|WS_VISIBLE,CRect(0,0,wndRct.Width(),wndRct.Height()),this,200);
...
void CSkinScrollWnd::OnTimer(UINT nIDEvent) 
{
	...
	si1.fMask=SIF_ALL;
	SCROLLINFO si2={0};
	if(m_sbVert.IsWindowVisible() && !m_sbVert.m_bDrag)
	{
	...
	if(m_sbHorz.IsWindowVisible() && !m_sbHorz.m_bDrag)
	{
