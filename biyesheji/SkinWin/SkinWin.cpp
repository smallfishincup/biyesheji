...
BOOL CSkinWin::LoadSkin( const char * skinfile )
{
	...
	if ( *buf != 0 )
	{
		strBmpFile = path + "/" + GetFileName( buf,1 );
		m_bmpDlg.LoadBitmap( strBmpFile );
	}

	GetPrivateProfileString( ps, "Top", "", buf, 1000, skinfile );
	strBmpFile = path + GetFileName( buf,1 );
	if ( *buf == 0 || !m_bmpTitle.LoadBitmap( strBmpFile ) )
		return FALSE;

	GetPrivateProfileString( ps, "Left", "", buf, 1000, skinfile );
	strBmpFile = path + GetFileName( buf,1 );
	if ( *buf == 0 || !m_bmpLeft.LoadBitmap( strBmpFile )) 
		return FALSE;

	GetPrivateProfileString( ps, "Right", "", buf, 1000, skinfile );
	strBmpFile = path + GetFileName( buf,1 );
	if ( *buf == 0 || !m_bmpRight.LoadBitmap( strBmpFile )) 
		return FALSE;

	GetPrivateProfileString( ps, "Bottom", "", buf, 1000, skinfile );
	strBmpFile = path + GetFileName( buf,1 );
	if ( *buf == 0 || !m_bmpBottom.LoadBitmap( strBmpFile )) 
		return FALSE;
...
{	
	...
	DrawTitle( pNewDC, m_BorderLeftWidth , 0, wr.Width() - m_BorderRightWidth - m_BorderLeftWidth + 1, state );
	DrawLeft( pNewDC, 0, 0, m_bmpLeft.Height(), state );
//	DrawRight( pNewDC, wr.Width() , 0, m_bmpRight.Height(), state );
	...
	//draw icon
	::DrawIconEx( pNewDC->GetSafeHdc(), m_BorderLeftWidth + 5, 5, (HICON)
